...
async function run() {
  ...
  for (const product of PRODUCTS) {
    ...
    try {
      ...
      // Nuke absolutely anything that looks like an overlay
      await page.evaluate(() => {
        const selectors = [
          '.truste_overlay', '.truste_box_overlay', '#truste-consent-track',
          '#cookie-consent-banner', '.cookie-banner', '[class*="cookie"]', '[id*="cookie"]',
          'iframe[title="TrustArc Cookie Consent"]', 'iframe[src*="trustarc"]',
          '.p-modal', '.p-overlay', '[role="dialog"]', '.p-sticky'
        ];
        ...
        // Also aggressively hide all `fixed` elements on the body just in case
