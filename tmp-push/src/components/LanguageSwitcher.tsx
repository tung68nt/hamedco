...
export default function LanguageSwitcher() {
  ...
  return (
    <div className="lang-switcher" ref={ref}>
      <button
        className="lang-btn-premium notranslate"
      ...
      {open && (
        <div className="lang-dropdown-premium">
          {LANGS.map((lang) => (
            <button
              key={lang.code}
              className={`lang-option-premium${lang.code === locale ? " active" : ""}`}
              onClick={() => switchLocale(lang.code)}
            >
              <span className="lang-flag">{lang.flag}</span>
              <span className="lang-text">{lang.label}</span>
              {lang.code === locale && (
                <svg className="check-icon" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
